<template>
  <div>
    ...
    <div v-if="!selectedTrain">Please select Train Line first</div>
    <div v-else>
      <!-- <sui-dropdown
        placeholder="Station"
        selection
      ...
      /> -->
      <div>
        <h3 v-if="!!selectedStation">
          Station - {{ selectedStation.station_name }}
        </h3>
        <TrainArrival
          v-for="arrival in getArrivals"
          :key="arrival.run_number"
          :arrival="arrival"
        />
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    getStations: function () {
      const stationMapIds = this.stations
        .filter((station) => station[this.selectedTrain])
        .map((station) => station.map_id)
    ...
    getStation: function () {
      this.selectedStation = this.stations.find(
        (station) => station.map_id === this.mapId
      )
    }
  }
}
